...
async function getRoomGiftCounts(userId) {
    ...
    try {
        const [
            ...
            // count opened gifts
            database.getQuery("SELECT COUNT(*) AS count FROM gifts WHERE receiver = ? AND status = 'active' AND has_opened = 1", [userId]),
            // check for any unopened gifts
            database.getQuery("SELECT COUNT(*) AS count FROM gifts WHERE receiver = ? AND status = 'active' AND has_opened = 0", [userId]),
            ...
            database.getQuery("SELECT COUNT(*) AS count FROM mystery_gifts WHERE receiver = ? AND has_opened = 0", [userId])
        ]);
...
async function getOpenedGiftCount(userId) {
    try {
        const row = await database.getQuery(
            "SELECT COUNT(*) AS count FROM gifts WHERE receiver = ? AND has_opened = 1",
            [userId]
        );
        return row?.count || 0;
    } catch (error) {
